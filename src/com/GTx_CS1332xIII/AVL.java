...
public class AVL<T extends Comparable<? super T>> {
    ...
    private AVLNode<T> _add(T data, AVLNode<T> currentNode){
        if (currentNode == null){
        ...
        if (currentNode.getData().compareTo(data) > 0){
            AVLNode<T> leftTree = _add(data, currentNode.getLeft());
            currentNode.setLeft(leftTree);
        } else if (currentNode.getData().compareTo(data) < 0){
            AVLNode<T> rightTree = _add(data, currentNode.getRight());
            currentNode.setRight(rightTree);
        }
    ...
    public T remove(T data) {
        ...
        root = _remove(data, root, dummy);
        size--;
        return dummy.getData();
    }

    private AVLNode<T> _remove(T data, AVLNode<T> currentNode, AVLNode<T> dummy){
        if (currentNode == null){
        ...
        if (currentNode.getData().compareTo(data) == 0){
            ...
            if (leftChild == null && rightChild == null){
                ...
                AVLNode<T> successorNode = new AVLNode<T>(null);
                AVLNode<T> modifiedTree = _successor(currentNode.getRight(), successorNode);
                currentNode.setRight(modifiedTree);
                currentNode.setData(successorNode.getData());
            }

        } else if (currentNode.getData().compareTo(data) > 0){
            AVLNode<T> l = _remove(data, currentNode.getLeft(), dummy);
            currentNode.setLeft(l);
        } else {
           AVLNode<T> r =  _remove(data, currentNode.getRight(), dummy);
           currentNode.setRight(r);
        }
        return balance(currentNode);
    };

    private AVLNode<T> _successor(AVLNode<T> currentNode, AVLNode<T> successorNode){
        if (currentNode.getLeft() == null){
            successorNode.setData(currentNode.getData());
            return currentNode.getRight();
        }
        currentNode.setLeft(_successor(currentNode.getLeft(), successorNode));
        return balance(currentNode);
    }
    ...
    private AVLNode<T> rotateLeft(AVLNode<T> currentNode) {
        AVLNode<T> r = currentNode.getRight();
        currentNode.setRight(r.getLeft());
        r.setLeft(currentNode);
    ...
    private AVLNode<T> rotateRight(AVLNode<T> currentNode) {
        AVLNode<T> l = currentNode.getLeft();
        currentNode.setLeft(l.getRight());
        l.setRight(currentNode);
        updateHeightAndBF(currentNode);
        updateHeightAndBF(l);
        return l;
    }
